...
def description():
    description_text = [        
        "Программа тестирования работы различных БД",
        "Для исследования и обучения   работы с разными БД,",
        "таких, как:", 
        "PostgreSQL,   SQLite,        MongoDB,   RethinkDB,",
        "QuestDb,      TimescaleDB,   Redis,       RabbitMQ",
        "создан тренажер, где возможен выбор типа БД и работы с ней.",
        "В данном приложении собрано несколько действующих примеров.",
        "Структура программы-демонстратора - проста и масштабируема,",
        "Это позволяет изменять ее возможности в разных применениях.",
    ]
    return render_template('description.html', title='Description of Application', description_text=description_text)
...
def about():
    ...
    return render_template('about.html', title='About', about_text=about_text)
...
def get_terminal_output():
    ...
    for i, row in enumerate(data[:QUESTDBNUMBROWSOUT]):
    ...
    if len(data) > QUESTDBNUMBROWSOUT:
        output.append("...")
